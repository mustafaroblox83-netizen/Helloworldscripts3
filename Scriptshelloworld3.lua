                        ...
                        if root and hrp then
                            ...
                            if ray and ray.Instance and not ray.Instance:IsDescendantOf(plr.Character) then
                                if not bb then
                                    ...
                                    New("TextLabel",{Size=UDim2.new(1,0,1,0),BackgroundTransparency=1,Text="🧱 "..plr.Name,TextColor3=C.Gold,TextSize=11,Font=Enum.Font.GothamBold},b)
                                end
                            else
...
Corner(v3b,5); Stroke(v3b,C.Purple,1)
New("TextLabel",{Size=UDim2.new(1,0,1,0),BackgroundTransparency=1,Text="⚡ V3",TextColor3=C.Purple,TextSize=11,Font=Enum.Font.GothamBold},v3b)

New("TextLabel",{Size=UDim2.new(0,440,0,24),Position=UDim2.new(0.5,-220,0.4,0),BackgroundTransparency=1,Text="RIVALS & UNIVERSAL  •  KEY NEEDED",TextColor3=C.TextSub,TextSize=12,Font=Enum.Font.Gotham},KeyScreen)

local KBG=New("Frame",{Size=UDim2.new(0,340,0,32),Position=UDim2.new(0.5,-170,0.5,0),BackgroundColor3=C.BG_Card,BorderSizePixel=0},KeyScreen)
...
local function BuildMain()
    ...
    local function SetTab(name)
        ...
        for n,btn in pairs(TabBtns) do
            if n==name then Tween(btn,{BackgroundColor3=C.AccentDim},0.2); btn.BackgroundTransparency=0; btn.TextColor3=C.Accent
    ...
    local hBtn=New("TextButton",{Size=UDim2.new(0,60,0,22),Position=UDim2.new(1,-130,0.5,-11),BackgroundColor3=C.Accent,Text="Head",TextColor3=Color3.fromRGB(255,200,200),TextSize=10,Font=Enum.Font.GothamBold,BorderSizePixel=0},aimCard); Corner(hBtn,5)
    ...
    Toggle(OP,"Auto 1v1 Detect","Auto1v1","Auto lock aim ke target 1v1",C.Purple)

    -- Target info
    local tCard=New("Frame",{Size=UDim2.new(1,0,0,62),BackgroundColor3=C.BG_Card,BorderSizePixel=0},OP)
    Corner(tCard,8); Stroke(tCard,C.Purple,1.5)
    local targetNameLbl=New("TextLabel",{Size=UDim2.new(1,-16,0,22),Position=UDim2.new(0,8,0,5),BackgroundTransparency=1,Text="🎯 Tidak ada target",TextColor3=C.Purple,TextXAlignment=Enum.TextXAlignment.Left,TextSize=13,Font=Enum.Font.GothamBold},tCard)
    ...
    Section(VP,"ESP")
    Toggle(VP,"ESP + Highlight","ESP","🎯 Ungu = Target 1v1 | 🔴 Merah = Player lain",C.Accent)
    Section(VP,"MOVEMENT")
    Toggle(VP,"Speed Hack","SpeedHack","WalkSpeed custom")
    Slider(VP,"🏃 Walk Speed",16,250,16,function(v) Settings.WalkSpeed=v; local _,_,hum=GetChar(); if hum and Toggles.SpeedHack then hum.WalkSpeed=v end end,"")
